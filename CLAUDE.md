...
cd apps/dure && npm run start -- tools                                 # RPC 메서드 목록
cd apps/dure && npm run start -- call <method> '<json_params>'         # RPC 메서드 호출
cd apps/dure && npm run start -- quote <stock_code>                    # KIS 현재가 조회
...
apps/dure/                                  apps/cluefin-rpc/
  ...
  stdio-jsonrpc-client.ts (child_process.spawn) dispatcher.py (method routing)
...
- 도구 실행: `npm run start -- call <method> <params>` CLI를 셸 커맨드로 호출
