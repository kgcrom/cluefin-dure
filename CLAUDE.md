...
broker CLI: `cd apps/broker && bun run start kis` (또는 `kiwoom`, `order`). 루트의 `.env` 파일을 `--env-file=../../.env`로 참조한다. `order add`는 인터랙티브 프롬프트(`@clack/prompts`)로 동작한다.
...
- 외부 런타임 의존성 최소화 — Bun 내장 API와 표준 fetch만 사용. 예외: trader는 Hono, broker는 @clack/prompts 사용
