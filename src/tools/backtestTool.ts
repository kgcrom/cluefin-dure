...
export const backtestTool: ToolDefinition<typeof parameters> = {
  name: "run_backtest",
  label: "백테스트 실행",
  description: "정의된 전략을 과거 데이터에 대해 백테스트합니다. CAGR, MDD, Sharpe 등 성과지표를 계산합니다.",
  parameters,
  async execute(_toolCallId, params: Params) {
    ...
    const tradeLog = tickers.slice(0, 3).flatMap((ticker: string) => [
      { date: startDate, ticker, action: "BUY", price: 100 + hash % 50, quantity: Math.floor(initialCapital / tickers.length / (100 + hash % 50)) },
      { date: endDate, ticker, action: "SELL", price: (100 + hash % 50) * (1 + cagr), quantity: Math.floor(initialCapital / tickers.length / (100 + hash % 50)) },
    ]);
