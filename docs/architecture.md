...
- `chat`에서는 백테스트를 실행하지 않습니다.
- 백테스트는 `backtest <strategyId>` CLI 명령으로만 직접 실행합니다.
...
- Equity Analysis: Universe → Fundamental + News → Strategy → Backtest → Critic
- Interactive Equity Analysis: Universe → Fundamental + News → Strategy 초안
