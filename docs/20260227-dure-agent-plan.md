...
- **도구 실행**: 기존 `npm run start -- call <method> <params>` CLI
...
Codex (LLM)
    ...
    ├─ 판단: 일봉 데이터 필요 → RSI 계산
    │
    ├─ $ npm run start -- call kis.basic_quote.stock_daily '{"stock_code":"005930"}'
    │   └─→ cluefin-rpc → KIS API → 일봉 데이터 반환
    │
    ├─ $ npm run start -- call ta.rsi '{"close":[...],"timeperiod":14}'
...
지침 내용:
- 세션 초기화 필수: `npm run start -- call session.initialize '{"broker":"kis"}'`
...
```sh
# 현재: 두 번 호출 필요
npm run start -- call session.initialize '{"broker":"kis"}'
