...
import { getCurrentPrice, getFinancials, getMarketAnnouncement, getPriceHistory } from "./providers/kis.js";
import type { CliExecutionResult, JsonValue, ToolResultDetails } from "./types.js";

function ok(provider: ToolResultDetails["provider"], result: CliExecutionResult): AgentToolResult<ToolResultDetails> {
  return {
...
function fail(provider: ToolResultDetails["provider"], error: unknown): AgentToolResult<ToolResultDetails> {
...
export default function (pi: ExtensionAPI) {
  pi.registerTool({
    ...
    async execute(_toolCallId, _params, signal) {
      try {
        ...
        const data = { cwd: result.cwd, command: result.command, env, cli: result.data } satisfies JsonValue;
        return {
          content: [{ type: "text" as const, text: JSON.stringify(data, null, 2) }],
          details: { provider: "market-data", cwd: result.cwd, command: result.command, data } satisfies ToolResultDetails,
        };
      } catch (error) {
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      market: Type.Optional(Type.Union([Type.Literal("J"), Type.Literal("NX"), Type.Literal("UN")])),
    }),
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      market: Type.Optional(Type.Union([Type.Literal("J"), Type.Literal("NX"), Type.Literal("UN")])),
      period: Type.Optional(Type.Union([Type.Literal("D"), Type.Literal("W"), Type.Literal("M"), Type.Literal("Y")])),
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      startDate: Type.Optional(Type.String({ description: "Date filter. CLI expects provider-specific format." })),
    }),
    async execute(_toolCallId, params, signal) {
      try {
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      corpClass: Type.Optional(Type.Union([Type.Literal("Y"), Type.Literal("K"), Type.Literal("N"), Type.Literal("E")])),
